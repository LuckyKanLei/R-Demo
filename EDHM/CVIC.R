...
CVIC <- setClass("CVIC", contains = "hm.list")
setMethod("ReferenceET", "CVIC", ReferenceET.PenmanMonteith)
setMethod("ET", "CVIC", ET.VIC)
setMethod("INTERCEPTION", "CVIC", INTERCEPTION.Gash)
setMethod("InfiltratRat", "CVIC", InfiltratRat.GreenAmpt)
setMethod("RUNOFF", "CVIC", RUNOFF.VIC)
setMethod("BASEFLOW", "CVIC", BASEFLOW.ARNO)
setMethod("GROUNDWATER", "CVIC", GROUNDWATER.VIC)
setMethod("ROUTE", "CVIC", ROUTE.IUHG2RES)
...
TimeInvariData <- list()
ModelData <- list()
class(ModelData) = class(StoreData) = class(TimeVariData) = class(TimeInvariData) <- c("CVIC", "hm.list", "list")
...
## model Data ####
PeriodData <- list()
class(PeriodData) <- c("CVIC", "hm.list", "list")
Ground <- list()
...
for (i in 2:PeriodN) {
  ...
  ## Intercept ####
  ICOut <- INTERCEPTION(PeriodData, Param)
  PeriodData <- mergeData(PeriodData, ICOut)
  ## base flow ####
